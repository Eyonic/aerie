...
type Plat = { key: string; label: string; kind: string; available: boolean; url: string | null; filename: string | null; sizeBytes: number };
...
export default function GetApps() {
  ...
  useEffect(() => {
    api.apps().then(d => setPlats(d.platforms)).catch(() => setPlats([])).finally(() => setLoading(false));
    return pwa.onChange(() => setCanInstall(pwa.canInstall()));
  }, []);
    ...
    <div className="animate-fade-in max-w-5xl">
          ...
          {plats.map(p => (
            <div key={p.key} className={cx('card p-5 card-hover', p.key === os && 'ring-1 ring-brand-500/40')}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="font-semibold text-white flex items-center gap-2">{p.label} {p.key === os && <Badge color="brand">Your device</Badge>}</p>
              ...
              {p.available ? (
                <>
                  <a href={p.url!} download className="btn-primary w-full justify-center">
                    <Icon.Download size={17} /> Download
                  </a>
                  <p className="text-[11px] text-slate-500 mt-2 text-center">{p.filename} · {formatBytes(p.sizeBytes)}</p>
                  {isWebViewApk(p) && (
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-6">
        ...
        <div className="card p-5">
          ...
          <ul className="text-sm muted space-y-1.5 list-disc pl-5">
            <li><b className="text-slate-300">Windows:</b> run the installer (.exe). It adds Aerie to your Start menu.</li>
            <li><b className="text-slate-300">Linux:</b> make the .AppImage executable and run it, or install the .deb.</li>
