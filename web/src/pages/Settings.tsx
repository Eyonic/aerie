...
function SyncStatusText({ folder, status }: { folder: DesktopSyncFolder; status?: DesktopSyncStatus }) {
      ...
      <div className="space-y-2">
        {folders.length === 0 ? (
          ...
          <div key={f.uri} className="flex items-center gap-3 rounded-xl bg-white/[0.025] border border-white/[0.05] p-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-white truncate">{f.label}</p>
              <p className="text-xs muted truncate">{status.running ? 'Syncing now...' : (status.lastResult || 'Waiting for next charge-time run')}</p>
            </div>
